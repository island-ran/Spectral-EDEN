...
// V3 keeps low expected gain as a ranking hint only.  Invalid policy inputs
// fail soft to 1.0 so they cannot accidentally erase a reachable candidate.
double ExpectedGainRankingScale(double expected_gain, double low_gain_epsilon,
                                double low_gain_scale);

struct RepeatPlanningObservation {
  int repeat_count = 0;
  double last_count_time = -1.0;
  std::uint64_t total_repeat_count = 0U;
};

// Counts repeated planning observations for diagnostics only.  The return
// type deliberately contains no frontier state or quarantine decision.
RepeatPlanningObservation ObserveRepeatedPlanning(
    ...
    double now, double minimum_count_interval = 0.5);

struct FirstTargetChoice {
  bool valid = false;
  bool within_regret = false;
  std::size_t baseline_index = 0U;
  double baseline_distance = std::numeric_limits<double>::infinity();
  double selected_distance = std::numeric_limits<double>::infinity();
  double regret = std::numeric_limits<double>::infinity();
};

// Distances and region IDs follow the original EOHDT order.  Only the first
// top_k entries are considered and ties preserve the baseline order.
...
// Returns distances.size() when no candidate is finite and restorable.
