...
TEST(SpectralV2PolicyTest, LowExpectedGainOnlyDownweightsRanking) {
  EXPECT_DOUBLE_EQ(ExpectedGainRankingScale(2.0, 1.0, 0.25), 1.0);
  EXPECT_DOUBLE_EQ(ExpectedGainRankingScale(1.0, 1.0, 0.25), 0.25);
  EXPECT_DOUBLE_EQ(ExpectedGainRankingScale(0.0, 1.0, 0.25), 0.25);
  EXPECT_DOUBLE_EQ(ExpectedGainRankingScale(0.0, 1.0, 0.0), 1.0e-3);
  EXPECT_DOUBLE_EQ(
      ExpectedGainRankingScale(std::numeric_limits<double>::quiet_NaN(),
                               1.0, 0.25),
      1.0);
}

TEST(SpectralV2PolicyTest, RepeatedPlanningIsDiagnosticOnly) {
  RepeatPlanningObservation observation =
      ObserveRepeatedPlanning(false, 0, -1.0, 0U, 0.0);
  EXPECT_EQ(observation.repeat_count, 1);
  EXPECT_EQ(observation.total_repeat_count, 0U);

  for (int repeat = 0; repeat < 15; ++repeat) {
    observation = ObserveRepeatedPlanning(
        true, observation.repeat_count, observation.last_count_time,
        observation.total_repeat_count, 0.5 * static_cast<double>(repeat + 1));
  }
  EXPECT_EQ(observation.repeat_count, 16);
  EXPECT_EQ(observation.total_repeat_count, 15U);
}

TEST(SpectralV2PolicyTest, FirstTargetSelectionOnlyUsesEohdtPrefix) {
  const std::vector<double> distances{100.0, 101.0, 80.0};
  const std::vector<int> regions{1, 0, 0};

  const FirstTargetChoice observe_only =
      ChooseFirstTargetFromEohdtPrefix(
          distances, regions, 1U, 0, 20.0, 0.10, 0.02);
  ASSERT_TRUE(observe_only.valid);
  EXPECT_EQ(observe_only.baseline_index, 0U);
  EXPECT_DOUBLE_EQ(observe_only.selected_distance, 100.0);
  EXPECT_TRUE(observe_only.within_regret);

  const FirstTargetChoice prefix_choice =
      ChooseFirstTargetFromEohdtPrefix(
          distances, regions, 2U, 0, 20.0, 0.10, 0.02);
  ASSERT_TRUE(prefix_choice.valid);
  EXPECT_EQ(prefix_choice.baseline_index, 1U);
  EXPECT_DOUBLE_EQ(prefix_choice.selected_distance, 101.0);
  EXPECT_NEAR(prefix_choice.regret, 0.01, 1.0e-12);
  EXPECT_TRUE(prefix_choice.within_regret);

  const FirstTargetChoice full_prefix =
      ChooseFirstTargetFromEohdtPrefix(
          distances, regions, 3U, 0, 20.0, 0.10, 0.02);
  ASSERT_TRUE(full_prefix.valid);
  EXPECT_EQ(full_prefix.baseline_index, 2U);
  EXPECT_DOUBLE_EQ(full_prefix.selected_distance, 80.0);
}

TEST(SpectralV2PolicyTest, FirstTargetRegretGuardRejectsLongerSuggestion) {
  const FirstTargetChoice choice =
      ChooseFirstTargetFromEohdtPrefix(
          {100.0, 103.0}, {1, 0}, 2U, 0, 50.0, 0.10, 0.02);
  ASSERT_TRUE(choice.valid);
  EXPECT_EQ(choice.baseline_index, 1U);
  EXPECT_NEAR(choice.regret, 0.03, 1.0e-12);
  EXPECT_FALSE(choice.within_regret);
}

TEST(SpectralV2PolicyTest, FailOpenChoosesNearestRestorableRawFrontier) {
  EXPECT_EQ(
      ChooseNearestFailOpenCandidate(
          {9.0, 3.0, 5.0}, {1U, 1U, 1U}),
      1U);
  EXPECT_EQ(
      ChooseNearestFailOpenCandidate(
          {1.0, 3.0, 5.0}, {0U, 1U, 1U}),
      1U);
  EXPECT_EQ(
      ChooseNearestFailOpenCandidate(
