...
void Takeoff(const std_msgs::EmptyPtr &msg){
    ...
    for(int i = 0; i < 30; i++){
        ...
        real_pt.position.z = (takeoff_pos_(2) + 0.5) * i / 30;
        // trajectory_point.position_W.z() = (takeoff_pos_(2) + 0.5) * i / 30;
        cmd.position.z = (takeoff_pos_(2) + 0.0) * i / 30;
...
int main(int argc, char** argv){
    ...
    nh_private.param(ns + "/Exp/TakeoffHold", hold_t, 3.0);
