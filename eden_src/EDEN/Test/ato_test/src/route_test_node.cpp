...
ros::Timer reload_timer_, dtg_timer_, route_timer_;
...
void ImgOdomCallback(const sensor_msgs::ImageConstPtr& img,
    ...
    if(!BM_->changed_pts_.empty()){
        ...
        eroi_->LoadNewFeasibleLrmVox(new_free_nodes_);
        eroi_->Robot_pos_ = cur_p;
...
void ReloadMap(const ros::TimerEvent &e){
    ...
    LRM_->SetNodeStatesBf(new_free_nodes_); // new_free_nodes_ not used
    new_free_nodes_.clear();
    // ROS_WARN("check2!");
    // LRM_->CheckThorough();
    BM_->changed_pts_.clear();
...
void RouteCallback(const ros::TimerEvent &e){
    if(!have_odom) return;
    vector<DTGPlus::h_ptr> route_h;
    ros::WallTime t1 = ros::WallTime::now();
    vector<Eigen::Vector3d> path1;
    double d1;
    DTG_->TspApproxiPlan(cur_p, route_h, path1, d1);
    double c = (ros::WallTime::now() - t1).toSec();
    cout<<"RouteCallback update time:"<<c<<endl;
}
...
int main(int argc, char** argv){
    ...
    dtg_timer_ = nh.createTimer(ros::Duration(0.4), &UpdateDTG);
    route_timer_ = nh.createTimer(ros::Duration(0.5), &RouteCallback);
