...
TEST(SpectralIterativeSolverTest, SolverSelectionHonorsFortyAndEightyLimits) {
  ...
  const SpectralResult eighty = router.Solve(MakeLargeIrregularChain(80U));
  ASSERT_TRUE(eighty.success()) << eighty.diagnostics.reason;
  EXPECT_EQ(eighty.diagnostics.solver_type,
            SpectralSolverType::SPARSE_BLOCK_INVERSE_ITERATION);

  const SpectralResult eighty_one =
      router.Solve(MakeLargeIrregularChain(81U));
  EXPECT_EQ(eighty_one.status, SpectralSolveStatus::TOO_MANY_NODES);
  EXPECT_EQ(eighty_one.diagnostics.solver_type, SpectralSolverType::NONE);
}
